...
override_prefs = {'ssl.keylog_file': "Non_Default\\Trace7\\sslkey.log"}

cap = pyshark.FileCapture("Non_Default\\Trace7\\non_default7.pcapng",
                          display_filter="(ssl.record.content_type==23 && tls)",
                          override_prefs=override_prefs)
...
for pckt in cap:
    ...
    if len(time_seq_list) != 0:
        diff_time = time_seq - time_seq_list[-1]
        byte_len = byte_len / diff_time
...
plt.title("Non Default Selection BPS vs Time Plot")
plt.xlabel("time")
...
fig3 = plt.figure(3)
plt.plot(time_seq_list, packet_len_list, linewidth=0.5, color='green')
plt.title("Non Default Selection Packet Length vs Time Plot")
plt.xlabel("time")
